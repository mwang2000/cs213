...
// IA32-32
  ...
  if (from_thread->state == TS_DYING) {
    ...
    // at this point another thread could dequeue from_thread and run it, so don't touch it after enqueuing
  }
...
void uthread_yield() {
  /* uthread_switch will take care of re-enqueuing this thread. */
